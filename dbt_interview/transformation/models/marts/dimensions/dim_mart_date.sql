...
extract(
    doy
    from date_day
) as day_of_year_number,
extract(
    week
    from date_day
) as week_of_year_number,
extract(
    month
    from date_day
) as month_of_year_number,
extract(
    quarter
    from date_day
) as quarter_of_year_number,
extract(
    year
    from date_day
) as year_number,
...
concat(
    datepart('Month', date_day::date),
    ' ',
    extract(
        year
        from date_day
    )
) as full_month_year --d.*
from dates_raw d